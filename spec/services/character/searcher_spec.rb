...
RSpec.describe Character::Searcher do
  ...
  it "searches names correctly" do
    results = Character::Searcher.new(templates: []).search(params: { name: 'a', search_name: true })
    expect(results).to match_array([@name])
  end

  it "searches screenname correctly" do
    results = Character::Searcher.new(templates: []).search(params: { name: 'a', search_screenname: true })
    expect(results).to match_array([@screenname])
  end

  it "searches nickname correctly" do
    results = Character::Searcher.new(templates: []).search(params: { name: 'a', search_nickname: true })
    expect(results).to match_array([@nickname])
  end

  it "searches name + screenname correctly" do
    results = Character::Searcher.new(templates: []).search(params: { name: 'a', search_name: true, search_screenname: true })
    expect(results).to match_array([@name, @screenname])
  end

  it "searches name + nickname correctly" do
    results = Character::Searcher.new(templates: []).search(params: { name: 'a', search_name: true, search_nickname: true })
    expect(results).to match_array([@name, @nickname])
  end

  it "searches nickname + screenname correctly" do
    results = Character::Searcher.new(templates: []).search(params: { name: 'a', search_nickname: true, search_screenname: true })
